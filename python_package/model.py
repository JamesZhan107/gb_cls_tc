...
def categorical_focal_loss(gamma=2., alpha=.25):
    ...
    def categorical_focal_loss_fixed(y_true, y_pred):
        ...
        loss = alpha * K.pow(y_true - y_pred, gamma) * cross_entropy
...
def generate_from_paths_and_labels(input_paths, labels, batch_size, input_size=(359,359),aug=False,mixup=False):
    if mixup:
...
def main(args):
    ...
    base_model = Xception(include_top=False, weights='imagenet', input_shape=(359,359,3))
    ...
    model.compile(
        ...
        # loss=[categorical_focal_loss(alpha=.5, gamma=2)],
        loss=categorical_crossentropy,
        metrics=['accuracy'])
...
#recompile again
    ...
    hist_fine_last = model.fit_generator(
        generator=generate_from_paths_and_labels(
            input_paths=train_input_paths,
            labels=train_labels,
            batch_size=args.batch_size_fine,
